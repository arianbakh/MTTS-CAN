...
def preprocess_raw_video(videoFilePath, dim=36, sample_dir=None):
    ...
    while success:
        ...
        if width > height:
          sub_image = img[:, round((width - height) / 2):round((height + width) / 2), :]
        elif height > width:
          sub_image = img[round((height - width) / 2):round((height + width) / 2), :, :]
        else:
          sub_image = img
        vidLxL = cv2.resize(img_as_float(sub_image), (dim, dim), interpolation = cv2.INTER_AREA)
